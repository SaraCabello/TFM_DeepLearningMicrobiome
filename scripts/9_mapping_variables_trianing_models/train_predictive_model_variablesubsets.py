...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 17 19:35:01 2019
...
def train_model_3hiddenlayers(X_train_df, y_train_df, X_test_df, y_test_df, n_iter, activation_function, encoder):
    #train model that predicts OTU composition from environmental data with 3 hidden layer between input layer
    #and coding layer 
    ...
    encoded1 = Dense (4, activation=activation_function)(input_layer)
    encoded2 = Dense (4, activation=activation_function)(encoded1)
    encoded3 = Dense (20, activation=activation_function)(encoded2)
...
def load_autoencoder(path):
    #load autoencoder model and its parts: encoder and decoder
    #useful only for architectures 4 and 5
...
def calculate_OTUs_mse(X_test_df, y_test_df, model, decoder):
    #calculate mse between original OTU composition (y_test_df) and the predicted one from the code 
    ...
    #predict OTU composition from the code (produced by the model) using the decoder part of autoencoder
    predicted_OTUs=decoder.predict(model.predict(normalized_xtest_list))
...
def calculate_smape(X_test_df, y_test_df, model, decoder):
...
def save_results(X_train_df, y_train_df, X_test_df, y_test_df, n_iter, variable_names, df_results, encoder, decoder):
    activation_function='tanh'

    #train model
    model, result = train_model_3hiddenlayers(X_train_df, y_train_df, X_test_df, y_test_df, n_iter, activation_function, encoder)
    
    #calculate error measurements
    mse_train = calculate_OTUs_mse(X_train_df, y_train_df, model, decoder)
    mse_test = calculate_OTUs_mse(X_test_df, y_test_df, model, decoder)
    smape_train = calculate_smape(X_train_df, y_train_df, model, decoder)
    smape_test = calculate_smape(X_test_df, y_test_df, model, decoder)
    
    #add results to the df
    ind = len(df_results.index) + 1
    df_results.loc[ind] = [str(variable_names), mse_train, mse_test, smape_train, smape_test]
    
    return model, result, df_results

#df to save error measurements of the trained models
df_results=pd.DataFrame(columns=['variables', 'mse_train', 'mse_test', 'smape_train', 'smape_test'])
...
path ='./models/autoencoder/architecture5/autoencoder2_5_10-6-100-1000.hdf5'
...
model, result, df_results = save_results(X_train_df, y_train_df, X_test_df, y_test_df, n_iter, variable_names, df_results, encoder, decoder)
model.save("./models/mapping_variables_study/predictive_model/predictor_maize2_all.hdf5", overwrite=True)
print(df_results)
...
for variable in variable_names:
    if variable != 'X.SampleID':
        ...
        model, result, df_results = save_results(X_train_df, y_train_df, X_test_df, y_test_df, n_iter, variable_names, df_results, encoder, decoder)
        print(df_results)
        file_model='./models/mapping_variables_study/predictive_model/predictor_maize2_-'+str(variable)+'.hdf5'
        print(file_model)
...
for i in range(1, len(variable_names)):
    ...
    for j in range(i+1,len(variable_names)):
        ...
        X_train_df, X_test_df, y_train_df, y_test_df = transform_data(X_train_df, X_test_df, y_train_df, y_test_df, option, n)
        model, result, df_results = save_results(X_train_df, y_train_df, X_test_df, y_test_df, n_iter, variable_names2, df_results, encoder, decoder)
        print(df_results)
        file_model='./models/mapping_variables_study/predictive_model/predictor_maize2_-'+str(variable1)+'&'+str(variable2)+'.hdf5'
        model.save(file_model, overwrite=True)
        X_train_df, y_train_df, X_test_df, y_test_df = load_subsets("./data/original_data/2otu_table2save_80.biom")
        variable_names=X_train_df.columns.tolist()
...
for i in range(1, len(variable_names)):
    ...
    for j in range(i+1,len(variable_names)):
        ...
        for k in range(j+1,len(variable_names)):
            ...
            X_train_df, X_test_df, y_train_df, y_test_df = transform_data(X_train_df, X_test_df, y_train_df, y_test_df, option, n)
            model, result, df_results = save_results(X_train_df, y_train_df, X_test_df, y_test_df, n_iter, variable_names3, df_results, encoder, decoder)
            print(df_results)
            file_model='./models/mapping_variables_study/predictive_model/predictor_maize2_-'+str(variable1)+'&'+str(variable2)+'&'+str(variable3)+'.hdf5'
            model.save(file_model, overwrite=True)
            X_train_df, y_train_df, X_test_df, y_test_df = load_subsets("./data/original_data/2otu_table2save_80.biom")
            variable_names=X_train_df.columns.tolist()
    ...
    for j in range(i+1,len(variable_names)):
        ...
        for k in range(j+1,len(variable_names)):
            ...
            for l in range(k+1, len(variable_names)):
                ...
                X_train_df, X_test_df, y_train_df, y_test_df = transform_data(X_train_df, X_test_df, y_train_df, y_test_df, option, n)
                model, result, df_results = save_results(X_train_df, y_train_df, X_test_df, y_test_df, n_iter, variable_names4, df_results, encoder, decoder)
                print(df_results)
                file_model='./models/mapping_variables_study/predictive_model/predictor_maize2_-'+str(variable1)+'&'+str(variable2)+'&'+str(variable3)+'&'+str(variable4)+'.hdf5'
                model.save(file_model, overwrite=True)
                X_train_df, y_train_df, X_test_df, y_test_df = load_subsets("./data/original_data/2otu_table2save_80.biom")
                variable_names=X_train_df.columns.tolist()
        ...
        for k in range(j+1,len(variable_names)):
            ...
            for l in range(k+1, len(variable_names)):
                ...
                for m in range(l+1, len(variable_names)):
                    ...
                    X_train_df, X_test_df, y_train_df, y_test_df = transform_data(X_train_df, X_test_df, y_train_df, y_test_df, option, n)
                    model, result, df_results = save_results(X_train_df, y_train_df, X_test_df, y_test_df, n_iter, variable_names5, df_results, encoder)
                    print(df_results)
                    file_model='./models/mapping_variables_study/predictive_model/predictor_maize2_-'+str(variable1)+'&'+str(variable2)+'&'+str(variable3)+'&'+str(variable4)+'&'+str(variable5)+'.hdf5'
                    model.save(file_model, overwrite=True)
                    X_train_df, y_train_df, X_test_df, y_test_df = load_subsets("./data/original_data/2otu_table2save_80.biom")
                    variable_names=X_train_df.columns.tolist()
